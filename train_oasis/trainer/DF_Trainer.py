...
class DiffusionForcingVideo(pl.LightningModule):
    ...
    def _build_model(self, model_ckpt):
        ...
        elif self.model_cfg.architecture == "oasis_dit_yarn":
            from train_oasis.model.dit_yarn import DiT_yarn
            self.diffusion_model = DiT_yarn(
                input_h=self.model_cfg.input_h,
                input_w=self.model_cfg.input_w,
                patch_size=self.model_cfg.patch_size,
                in_channels=self.model_cfg.in_channels,
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        if batch_idx % self.cfg.save_video_every_n_step == 0 and self.logger:
            ...
            if self.global_rank == 0:
                ...
                print("Logging training video done.")
        return output_dict
    ...
    def validation_step(self, batch, batch_idx, namespace="validation") -> STEP_OUTPUT:
        ...
        pbar = tqdm(total=n_frames, initial=curr_frame, desc="Sampling", disable=(self.global_rank != 0))
    ...
    def on_validation_epoch_end(self, namespace="validation") -> None:
        ...
        if self.logger:
            log_video(
                ...
                logger=self.logger.experiment,
                prefix=f"video_{self.global_rank}"
            )
