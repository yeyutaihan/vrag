...
class LlamaYaRNScaledRotaryEmbedding(torch.nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, scale=1, original_max_position_embeddings=2048, extrapolation_factor=1, attn_factor=1, beta_fast=32, beta_slow=1, finetuned=False, device=None):
        ...
        # print(f"shape of inv_freq: {self.inv_freq.shape}")
        freqs = torch.einsum("i,j->ij", t, self.inv_freq)
        # print(f"shape of freqs: {freqs.shape}")
        # Different from paper, but it uses a different permutation in order to obtain the same calculation
        emb = torch.cat((freqs, freqs), dim=-1)
        dtype = torch.get_default_dtype()
        # print(f"shape of emb/cos/sin: {emb.shape}")
    ...
    def forward(self, x, seq_len=None):
        ...
        if seq_len > self.max_seq_len_cached:
            ...
            # print(f"shape of emb/cos/sin within forward: {emb.shape}")
        ...
        # print(f"shape of self.cos_cached[:seq_len] within forward2: {self.cos_cached[:seq_len].shape}")
        # print(f"self.mscale: {self.mscale}")
        return (
            self.cos_cached[:seq_len].to(dtype=x.dtype),
            self.sin_cached[:seq_len].to(dtype=x.dtype),
        )
